...
namespace Ferretto.WMS.Modules.ItemLists
{
    public class ItemListRowExecuteViewModel : BaseDialogViewModel<ItemListRowExecutionRequest>
    {
        ...
        public ItemListRowExecuteViewModel()
        {
            this.Initialize();
        }
        ...
        private async Task ExecuteListRowAsync()
        {
            ...
            if (result.Success)
            {
                this.EventService.Invoke(new StatusPubSubEvent(App.Resources.ItemLists.ItemListRowRequestAccepted, StatusType.Success, result.ShowToast));
                this.Disappear();
            }
            else
            {
