...
namespace Ferretto.VW.Drivers.Inverter
{
    public class Telegram
    {
        ...
        public byte[] BuildWritePacket(
            byte write_SystemIndex,
            byte write_DataSetIndex,
            short write_ParamID,
            ValueDataType write_DataType,
            object value)
        {
            var nBytesPayLoad = 0;
            switch (write_DataType)
            {
                ...
                case ValueDataType.String:
                    nBytesPayLoad = Convert.ToString(value).Length;
                    break;
            }
        ...
        public void ParseDataBuffer(byte[] telegram, int nBytes, out bool error)
        {
            ...
            parameterNo.Reverse();
            this.ParameterIdFromParse = ParameterIdClass.ValueToParameterIdCode(BitConverter.ToInt16(parameterNo, 0));

            // ParseDataBuffer_Data
            var nBytesPayLoad = nBytes - 6;
            if (nBytesPayLoad == 1) // type is byte
            {
                this.RetValueFromParse = telegram[6];
            }
            if (nBytesPayLoad == 2) // type is uint16
            {
            ...
            if (nBytesPayLoad == 4) // type is int32
            {
            ...
            if (nBytesPayLoad > 4) // type is string
            {
