    ...
    internal sealed class MissionOperationsService : IMissionOperationsService, IDisposable
    {
        ...
        public async Task<bool> IsMultiMachineAsync(int missionId)
        {
            try
            {
                ...
                var currentMission = allMissionsList.ToList().Find(x => x.Code == this.ActiveWmsMission.Operations.FirstOrDefault().ItemListCode && x.Status == ItemListStatus.Executing);
