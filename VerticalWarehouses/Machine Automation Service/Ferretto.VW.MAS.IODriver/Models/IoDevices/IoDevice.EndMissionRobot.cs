...
using Ferretto.VW.MAS.IODriver.StateMachines.EndMissionRobot;
...
namespace Ferretto.VW.MAS.IODriver
{
    internal partial class IoDevice
    {
        ...
        public void ExecuteEndMissionRobot(FieldCommandMessage receivedMessage)
        {
            this.logger.LogTrace("1:Method Start");

            if (receivedMessage.Data is IEndMissionRobotFieldMessageData messageData)
            {
                if (this.CurrentStateMachine != null)
                {
                    this.logger.LogError($"ExecuteEndMissionRobot: Io Driver already executing operation {this.CurrentStateMachine.GetType().Name}");
                ...
                {
                    this.CurrentStateMachine = new EndMissionRobotStateMachine(
                        messageData.Enable,
                        this.ioCommandQueue,
                        this.ioStatus,
                        this.deviceIndex,
