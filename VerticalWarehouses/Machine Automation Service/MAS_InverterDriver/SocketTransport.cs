...
namespace Ferretto.VW.InverterDriver
{
    public class SocketTransport : ISocketTransport, IDisposable
    {
        ...
        public async Task<bool> ConnectAsync()
        {
            ...
            {
                throw new ArgumentNullException( nameof( inverterAddress ), $"{nameof( inverterAddress )} can't be null" );
            }

            if(this.inverterAddress.AddressFamily != AddressFamily.InterNetwork)
            {
                throw new ArgumentException( "Inverter Address is not a valid IPV4 address", nameof( inverterAddress ) );
            }

            if(this.sendPort == 0)
            {
                throw new ArgumentNullException( nameof( sendPort ), $"{nameof( sendPort )} can't be zero" );
            }

            if(this.sendPort < 1024 || this.sendPort > 65535)
            {
                throw new ArgumentOutOfRangeException( nameof( sendPort ), $"{nameof( sendPort )} value must be between 1204 and 65535" );
            }

            if(this.transportClient != null || this.transportStream != null)
            {
                throw new InverterDriverException( "Socket Transport is already open", InverterDriverExceptionCode.SocketOpen );
            }
            ...
            {
                throw new InverterDriverException( "Failed to create Transport Socket client", InverterDriverExceptionCode.TcpClientCreationFailed, ex );
            }
            ...
            catch(Exception ex)
            {
                ...
                throw new InverterDriverException( "Failed to connect to Inverter Hardware", InverterDriverExceptionCode.TcpInverterConnectionFailed, ex );
            }
            ...
            catch(Exception ex)
            {
                this.transportClient?.Dispose();
                this.transportClient = null;
                throw new InverterDriverException( "Failed to retrieve socket communication stream", InverterDriverExceptionCode.GetNetworkStreamFailed, ex );
            }
        ...
        public async Task<byte[]> ReadAsync( CancellationToken stoppingToken )
        {
            ...
            {
                throw new InverterDriverException( "Transport Stream is null", InverterDriverExceptionCode.UninitializedNetworkStream );
            }

            if(!this.transportStream.CanRead)
            {
                throw new InverterDriverException( "Transport Stream not configured for reading data", InverterDriverExceptionCode.MisconfiguredNetworkStream );
            }
            ...
            {
                throw new InverterDriverException( "Error reading data from Transport Stream", InverterDriverExceptionCode.NetworkStreamReadFailure, ex );
            }
        ...
        public async Task WriteAsync( byte[] inverterMessage, CancellationToken stoppingToken )
        {
            ...
            {
                throw new InverterDriverException( "Transport Stream not configured for sending data", InverterDriverExceptionCode.MisconfiguredNetworkStream );
            }
