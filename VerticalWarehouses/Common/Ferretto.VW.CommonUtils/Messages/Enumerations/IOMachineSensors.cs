namespace Ferretto.VW.CommonUtils.Enumerations
{
    public enum IOMachineSensors
    {
        #region Remote IO 1 sensors

        RunningState = 0,

        MushroomEmergencyButtonBay1 = 1,

        MicroCarterLeftSide = 2,

        MicroCarterRightSide = 3,

        AntiIntrusionBarrierBay1 = 4,

        LUPresentInBay1 = 5,

        LUPresentMiddleBottomBay1 = 6,

        InverterInFault1 = 7,

        ElevatorEngineSelectedBay1 = 8,

        CradleEngineSelectedBay1 = 9,

        LuPresentInOperatorSide = 10,

        LuPresentInMachineSide = 11,

        ProfileCalibrationBay1 = 12,

        AntiIntrusionBarrier2Bay1 = 13,

        TrolleyOptionBay1 = 14,

        RobotOptionBay1 = 15,

        #endregion

        #region Remote IO 2 sensors

        Free1Bay2 = 16,

        MushroomEmergencyButtonBay2 = 17,

        AntiIntrusionBarrierBay2 = 20,

        LUPresentInBay2 = 21,

        LUPresentMiddleBottomBay2 = 22,

        Free2Bay2 = 23,

        Free3Bay2 = 24,

        Free4Bay2 = 25,

        Free5Bay2 = 26,

        Free6Bay2 = 27,

        ProfileCalibrationBay2 = 28,

        AntiIntrusionBarrier2Bay2 = 29,

        TrolleyOptionBay2 = 30,

        RobotOptionBay2 = 31,

        #endregion

        #region Remote IO 3 sensors

        Free1Bay3 = 32,

        MushroomEmergencyButtonBay3 = 33,

        AntiIntrusionBarrierBay3 = 36,

        LUPresentInBay3 = 37,

        LUPresentMiddleBottomBay3 = 38,

        Free2Bay3 = 39,

        Free3Bay3 = 40,

        Free4Bay3 = 41,

        Free5Bay3 = 42,

        Free6Bay3 = 43,

        ProfileCalibrationBay3 = 44,

        AntiIntrusionBarrier2Bay3 = 45,

        TrolleyOptionBay3 = 46,

        RobotOptionBay3 = 47,

        #endregion

        #region Main ANG Inverter sensors

        PowerOnOff = 48,

        FastStop = 49,

        ZeroVerticalSensor = 50,

        FreeAng1 = 51,

        FreeAng2 = 52,

        ElevatorOverrun = 53,

        FreeAng3 = 54,

        ZeroPawlSensor = 55,

        #endregion

        #region Chain ANG Inverter sensors

        ANGChainPowerOnOff = 56,

        ANGChainS2IND = 57,

        ZeroPawlSensorOneTon = 58,

        ANGChainS4IND = 59,

        ANGChainS5IND = 60,

        ANGChainS6IND = 61,

        ANGChainMF3ID = 62,

        ANGChainMF2ID = 63,

        #endregion

        #region Bay1 AGL Inverter sensors

        AGLPowerOnOffBay1 = 64,

        AGLNormalFunctionBay1 = 65,

        AGLSensorAShutterBay1 = 66,

        AGLSensorBShutterBay1 = 67,

        AGLFree1Bay1 = 68,

        AGLFree2Bay1 = 69,

        AGLFree3Bay1 = 70,

        AGLFree4Bay1 = 71,

        #endregion

        #region Bay1 ACU Inverter sensors

        ACUBay1STO = 72,

        ACUBay1S2IND = 73,

        ACUBay1S3IND = 74,

        ACUBay1S4IND = 75,

        ACUBay1S5IND = 76,

        ACUBay1S6IND = 77,

        ACUBay1MF1ID = 78,

        ACUBay1EMS1IND = 79,

        #endregion

        #region Bay2 AGL Inverter sensors

        AGLPowerOnOffBay2 = 80,

        AGLNormalFunctionBay2 = 81,

        AGLSensorAShutterBay2 = 82,

        AGLSensorBShutterBay2 = 83,

        AGLFree1Bay2 = 84,

        AGLFree2Bay2 = 85,

        AGLFree3Bay2 = 86,

        AGLFree4Bay2 = 87,

        #endregion

        #region Bay2 ACU Inverter sensors

        ACUBay2STO = 88,

        ACUBay2S2IND = 89,

        ACUBay2S3IND = 90,

        ACUBay2S4IND = 91,

        ACUBay2S5IND = 92,

        ACUBay2S6IND = 93,

        ACUBay2MF1ID = 94,

        ACUBay2EMS1IND = 95,

        #endregion

        #region Bay3 AGL Inverter sensors

        AGLPowerOnOffBay3 = 96,

        AGLNormalFunctionBay3 = 97,

        AGLSensorAShutterBay3 = 98,

        AGLSensorBShutterBay3 = 99,

        AGLFree1Bay3 = 100,

        AGLFree2Bay3 = 101,

        AGLFree3Bay3 = 102,

        AGLFree4Bay3 = 103,

        #endregion

        #region Bay3 ACU Inverter sensors

        ACUBay3STO = 104,

        ACUBay3S2IND = 105,

        ACUBay3S3IND = 106,

        ACUBay3S4IND = 107,

        ACUBay3S5IND = 108,

        ACUBay3S6IND = 109,

        ACUBay3MF1ID = 110,

        ACUBay3EMS1IND = 111,

        NoValue = 112,

        #endregion
    }
}
