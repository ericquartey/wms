...
namespace Ferretto.Common.BLL.Interfaces
{
  public interface IEventService
  {
    ...
    void Subscribe<TEventArgs>(Action<TEventArgs> action, string token, bool keepSubscriberReferenceAlive) where TEventArgs : IEventArgs;
    /// <summary>
    /// Subscribes a delegate to an event.
    /// </summary>
    /// <typeparam name="TEventArgs">The type of the event arguments.</typeparam>
    /// <param name="action">The delegate that gets executed when the event is raised.</param>
    void Subscribe<TEventArgs>(Action<TEventArgs> action) where TEventArgs : IEventArgs;
